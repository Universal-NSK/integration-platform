...
from nashdom_sync.settings.models import SyncSettings
...
class SettingsProvider:
    ...
    def provide(self) -> SyncSettings:
        ...
        app_settings = _load_toml(self._paths.config_file(_APP_CONFIG_NAME))
        path_settings = _load_toml(self._paths.program_data_file(_PATHS_CONFIG_NAME))
        merged_settings = _merge_settings(app_settings, path_settings)
        resolved_settings = _resolve_paths(merged_settings, self._paths)

        try:
            return SyncSettings.parse_obj(resolved_settings)
        except ValidationError as exc:
            raise ConfigurationError(
                f"Конфигурация синхронизации не прошла валидацию: {exc}"
            ) from exc
...
def _resolve_paths(
    ...
    for field_name in _PATH_FIELDS:
        ...
        try:
            resolved_browser[field_name] = paths.program_data_path(Path(raw_path))
